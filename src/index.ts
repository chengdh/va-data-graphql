...
const defaultOptions = {
  ...
  introspection: {
    operationNames: {
      ...
      [GET_TREE]: (resource) => `all${pluralize(resource.name)}`,
      [GET_NODES]: (resource) => `all${pluralize(resource.name)}`,
      [MOVE_NODE]: (resource) => `update${resource.name}`,
    },
    exclude: undefined,
    include: undefined,
  },
...
const handleError = (error: ApolloError) => {
  if (error?.networkError as ServerError) {
    throw new Error((error?.networkError as ServerError)?.message);
  }
...
const defaultDataProvider = {
  ...
  getTree: () => Promise.resolve({ data: [] }), // avoids adding a context in tests
  getNodes: () => Promise.resolve({ data: [] }), // avoids adding a context in tests
  moveNode: () => Promise.resolve({ data: null }), // avoids adding a context in tests
};
